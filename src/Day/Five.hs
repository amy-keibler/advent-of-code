...
nextInstructionsState fn (Instructions current backward forward)
  ...
  | current < 0 = if length backward < abC then Nothing else Just (Instructions (head $ drop (abC - 1) backward)
                                                                  (drop abC backward)
                                                                  (reverse (take (abC - 1) backward) ++ (fn current) : forward))
  | otherwise = if length forward < abC then Nothing else Just (Instructions (head $ drop (abC - 1) forward)
